--SQL HOMEWORK 4
--Attempted submission SEPT 21 2024
...
--Windowed Functions
--1. Write a query that selects from the customer_purchases table and numbers each customer’s visits to the farmer’s market (labeling each market date with a different number). 
--Each customer’s first visit is labeled 1, second visit is labeled 2, etc. 

--You can either display all rows in the customer_purchases table, with the counter changing on each new market date for each customer, or select only the unique market dates per customer 
...
--2. Reverse the numbering of the query from a part so each customer’s most recent visit is labeled 1, then write another query that uses this one as a subquery (or temp table) and filters the results to 
...
SELECT CP.*
	,COUNT(product_id) 
	OVER (PARTITION BY customer_id, product_id) AS product_purchases_count
...
SELECT market_date, total_sales FROM ranked_sales WHERE rank_desc = 1
UNION
SELECT market_date, total_sales FROM ranked_sales WHERE rank_asc = 1;
